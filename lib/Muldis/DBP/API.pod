...
Muldis.DBP.API -
Virtual Database Management System for C#/.Net

=head1 VERSION

This document is Muldis.DBP.API version 0.1.0.-9 for C#/.Net.
...
DBP> (B<MDBP>) library API for C#/.Net, authority C<http://muldis.com>,
...
concrete C#/.Net library named C<Muldis.DBP.API>; however,
no C#/.Net library with that name actually exists, and it never should
exist; thus C<Muldis.DBP.API> is a I<virtual> library.

An actual concrete C#/.Net library that implements and is fully conformant
...
by a C#/.Net application would be mediated by at least one I<procedure>.
...
.Net has a direct C<String> analogy which is provided by the .Net
class C<System.Collections.Generic.List> where every element is an integer.
...
.Net doesn't have a native C<Interval> analogy.
...
A typical .Net application would use a distinct C<Process> object for each
...
the DBMS and main .Net environments.  The way to represent an in-DBMS
